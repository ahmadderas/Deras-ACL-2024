...
const Products = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        // Pass the userId as a query parameter or in the request headers
        const response = await fetch(`/api/seller/view-products/${userId}`);
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      const response = await fetch(`/api/seller/add-product/${userId}`, {
        method: 'POST',
        headers: {
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      const response = await fetch(`/api/seller/update-product/${newProduct._id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleArchiveToggle = async (product) => {
    try {
      const response = await fetch(`/api/seller/${product.isArchived ? 'unarchive' : 'archive'}-product/${product._id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
          ...
          <div>
            {filteredProducts.map((product) => (
              <div key={product._id} className="product-card">
                {/* Log product.seller to the console */}
                <h3>{product.name}</h3>
                <p>{product.description}</p>
                <p>Price: ${product.price}</p>
                <p>Available Count: {product.availableCount}</p>
                <p>Rating: {product.ratings}</p>
                <p>Status: {product.isArchived ? 'Archived' : 'Active'}</p> {/* Display status */}
                
                {/* Show buttons only if the product belongs to the logged-in user */}
